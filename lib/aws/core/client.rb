      ...
      def log_response response
        ...
        if defined?(LOGGER)
          if response.duration > 0.1
            LOGGER.error "Dynamo Latency > 0.1 (latency: #{response.duration}) for #{response.throttled? ? 'throttled' : 'unthrottled'} request. RequestId: #{response.http_response.headers['x-amzn-requestid']} Response: #{response.inspect}, Request: #{response.http_request.inspect}"
          end
        end
        nil
      end
